...
import com.daxiang.lottery.entity.ScoreFootData;
 ...
 */
public class ScoreFootAdapter extends BaseAdapter {
    String[] weekDays = {"周日", "周一", "周二", "周三", "周四", "周五", "周六"};
   List<ScoreFootData.MatchesBean> data;
    HashMap<String, String> map = new HashMap<>();
    DbManager dbManager = new DbManager();
    CollectionListener mCollectionListener;
    public ScoreFootAdapter( List<ScoreFootData.MatchesBean> data,CollectionListener mCollectionListener) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final ScoreFootData.MatchesBean matchesBean = data.get(position);
        ...
        String[] split = start_time.split("T|\\+");
        mChildHolder.mTv_start_time.setText(split[0] + " " + split[1]);
        mChildHolder.mTv_host_name.setText(matchesBean.getHost_name());
        mChildHolder.mTv_away_name.setText(matchesBean.getAway_name());
        if ("完".equals(matchesBean.getStatus())) {
            mChildHolder.mHost_score.setText(matchesBean.getHost_score());
            mChildHolder.mAway_score.setText(matchesBean.getAway_score());
        ...
        mChildHolder.mCollection.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                colletionDb.setMatch_id(matchesBean.getMatch_id()+"");
                if (map.containsKey(matchesBean.getMatch_id())) {
                    try {
                        dbManager.deleteWhere(matchesBean.getMatch_id()+"");
                    } catch (DbException e) {
                        e.printStackTrace();
                    }
