...
public class KeyboardUtil {
    ...
    private boolean isBuy = false;//true加载的是购买的键盘，false加载的是跟单的键盘

    public KeyboardUtil(Activity activity, MyKeyBoardView mMyKeyBoardView, boolean isBuy) {
        this.mActivity = activity;
        this.mIfRandom = false;
        this.isBuy = isBuy;
        this.mKeyboardView = mMyKeyBoardView;
        if (isBuy)//购买的时候显示预约
            mKeyboardNumber = new Keyboard(mActivity, R.xml.keyboardbuy);
        else//跟单的时候显示跟单
            mKeyboardNumber = new Keyboard(mActivity, R.xml.keyboardnumber);
        //mKeyboardView = (MyKeyBoardView) mActivity.findViewById(R.id.keyboard_view);
    }
