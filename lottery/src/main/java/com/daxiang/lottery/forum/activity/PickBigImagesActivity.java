...
import com.daxiang.lottery.forum.selfview.ZoomImageView;
...
public class PickBigImagesActivity extends Activity implements ViewPager.OnPageChangeListener, View.OnClickListener{
    ...
    protected void initData() {
        ...
        if (getChooseStateFromList(currentPic)){
            iv_choose_state.setImageResource(R.mipmap.image_choose);
        }else{
            iv_choose_state.setImageResource(R.mipmap.image_not_chose);
        }
    ...
    public void onClick(View view) {
        ...
        if(getChooseStateFromList(currentPic)){
            if (last_pics <= 0){
                toggleChooseState(currentPic);
                Toast.makeText(this, String.format("你最多只能选择%1$d张照片", total_pics), Toast.LENGTH_SHORT).show();
                return ;
            }
