        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, ((WindowManager) (LotteryApp.getInstance().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager) (LotteryApp.getInstance().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth());
                    ...
                    String hash = DisplayUtil.md5(path+"_"+width+"_"+height);
                    File file = new File(DisplayUtil.getDataPath());
                    if (!file.exists())
                        file.mkdirs();
                    //临时文件的文件名
                    String tempPath = DisplayUtil.getDataPath() + hash + ".temp";
